...
export default function RegisterPage() {
  ...
  return (
    <div>
      ...
      <div className="inline-flex items-center gap-2 bg-brand-surface border border-brand-teal/20 px-3 py-1.5 rounded-full mb-3">
        <span className="w-1.5 h-1.5 rounded-full bg-brand-green animate-pulse" />
        <span className="text-xs font-medium text-brand-teal">Free account — no credit card</span>
      </div>

      <h1 className="font-display text-2xl font-bold text-brand-ink mb-1">Create your account</h1>
      <p className="text-gray-400 text-sm mb-1">अपना मोबाइल नंबर दर्ज करें</p>
      <p className="text-gray-500 text-sm mb-4">We'll send a 6-digit OTP to verify your number.</p>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="mb-4">
          ...
          <input id="name" type="text" autoComplete="name" placeholder="Ramesh Kumar" value={name}
            onChange={e => setName(e.target.value)}
            className="w-full px-4 py-3 rounded-2xl border-2 border-gray-200 focus:border-brand-teal outline-none text-sm text-brand-ink bg-white placeholder-gray-300 transition-all" />
        </div>
